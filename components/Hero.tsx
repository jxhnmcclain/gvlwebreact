...
const Hero = () => {
  ...
  useEffect(() => {
    ...
    // This prevents drag initialization from reading unstable layout coordinates on mount
  ...
  const cardProps = {
    ...
    dragElastic: 0.2,
    dragSnapToOrigin: false, // Ensure they return if thrown too far, helps with stability
    whileHover: { scale: 1.25, cursor: isDraggable ? 'grab' : 'pointer', zIndex: 50 },
    whileTap: { scale: 1.20, cursor: isDraggable ? 'grabbing' : 'default' },
  ...
  return (
    <section 
      ref={containerRef}
      className="relative w-full pt-40 pb-20 md:pt-48 md:pb-32 px-4 md:px-8 bg-white overflow-hidden h-auto min-h-screen md:min-h-0 md:h-[95vh] md:max-h-[1200px] flex flex-col justify-center"
    >
      <BubbleBackground className="absolute inset-0 pointer-events-none z-0" interactive={true} />
      ...
      >
        ...
        >
          ...
          >
             ...
             <div 
               ...
               className="absolute top-1/2 left-1/2 transform -translate-x-1/2 -translate-y-1/2 bg-white px-4 py-1.5 md:px-6 md:py-2 rounded-full border border-black text-xs md:text-sm font-bold shadow-sm z-10 pointer-events-auto cursor-pointer select-none whitespace-nowrap hover:bg-gray-100 transition-colors"
             >
               redes sociales
             </div>
          </motion.div>
        ...
        >
          <div className="bg-gvl-cream/80 backdrop-blur-sm border-2 border-black rounded-[2rem] md:rounded-[2.5rem] p-6 md:p-12 text-center shadow-xl w-full relative">
            ...
            <h1 className="text-2xl md:text-5xl font-bold leading-tight mb-6 md:mb-8 tracking-tight">
              Creamos <span className="underline decoration-4 decoration-gvl-yellow">contenido</span> y diseño para marcas con personalidad
            </h1>

            <button 
              onClick={() => navigate('/contacto')}
              className="bg-white border-2 border-black rounded-full px-6 py-2 md:px-8 md:py-3 text-base md:text-lg font-bold flex items-center gap-2 mx-auto hover:shadow-[4px_4px_0px_0px_rgba(0,0,0,1)] transition-all active:translate-y-1 active:shadow-none"
            >
              conversemos
           ...
           <motion.div 
            ...
            >
             ...
             <div 
               ...
               className="absolute top-1/2 left-1/2 transform -translate-x-1/2 -translate-y-1/2 bg-white px-4 py-1.5 md:px-6 md:py-2 rounded-full border border-black text-xs md:text-sm font-bold shadow-sm z-10 pointer-events-auto cursor-pointer select-none hover:bg-gray-100 transition-colors"
             >
               branding
             </div>
          </motion.div>
           ...
           <motion.div 
            ...
            >
             ...
             <div 
               ...
               className="absolute top-1/2 left-1/2 transform -translate-x-1/2 -translate-y-1/2 bg-white px-4 py-1.5 md:px-6 md:py-2 rounded-full border border-black text-xs md:text-sm font-bold shadow-sm whitespace-nowrap z-10 pointer-events-auto cursor-pointer select-none hover:bg-gray-100 transition-colors"
             >
            ...
            >
               ...
               <div className="absolute inset-0 flex items-center justify-center bg-black/10 pointer-events-none">
                  <div 
                    ...
                    className="bg-white px-4 py-1.5 md:px-6 md:py-2 rounded-full border border-black text-xs md:text-sm font-bold shadow-sm flex items-center gap-2 pointer-events-auto cursor-pointer select-none hover:bg-gray-100 transition-colors"
                  >
                     websites
                  </div>
               </div>
